...
const StateMachine = (function () {
  ...
  var statemachine = new sm.StateMachineAsync.Builder('lambda state machine', states, (creator, callback) => createStateFromS3(creator, callback))
    .withPersistance((state, data, callback) => {
      persistStateToS3(state, data, callback)
    })
    .build()

  function createStateFromS3 (creator, callback) {
  ...
  function persistStateToS3 (state, data, callback) {
